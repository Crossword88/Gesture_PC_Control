...
def show():                                     #showing the webcam frame with model predictions
    while True:
...
def controller():                                #entire code for mouse & volume control
    while True:
        sleep(0.0001)
        if CLASS == VOLUME_GESTURE:
            if gesture_center < 350:
...
def watcher():
    ...
    while True:
        ...
        result = model.predict(frame_to_model, conf = 0.4, device = 'cpu', verbose = False)[0]
        for box, cls, conf  in zip(result.boxes.xyxy, result.boxes.cls, result.boxes.conf):
            x1, y1, x2, y2 = box

            if cls == VOLUME_GESTURE:
                gesture_center = (y1 + y2) / 2
